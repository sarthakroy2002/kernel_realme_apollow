...
static int _present_fence_release_worker_thread(void *data)
{
	...
	while (1) {
		...
		#ifdef OPLUS_BUG_STABILITY
		/* Zhijun.Ye@PSW.MM.Display.LCD.Stability 2020/10/29,
		...
		if (!islcmconnected && !primary_display_is_video_mode()) {
			...
			#ifdef OPLUS_BUG_STABILITY
			/* Zhijun.Ye@PSW.MM.Display.LCD.Stability 2020/09/08,
...
int primary_display_suspend(void)
{
	...
	*/
	...
	/* Zhijun.Ye@PSW.MM.Display.LCD.Stability 2020/10/26, Add for fingerpress */
	if (oplus_display_fppress_support) {
		ds_rec_fpd = false;
		doze_rec_fpd = false;
	}
