...
static imgsensor_info_struct imgsensor_info = {
       .sensor_id = GC02M1B_SENSOR_ID_MACRO,        //record sensor id defined in Kd_imgsensor.h
...
kal_uint16 addr_data_pair_init_macro[] = {
/*system*/
0xfc,0x01,
0xf4,0x41,
0xf5,0xc0,
...
kal_uint16 addr_data_pair_preview_macro[] = {
	0xfe, 0x00,
	0x3e, 0x90,
};

kal_uint16 addr_data_pair_capture_macro[] = {
	0xfe, 0x00,
	0x3e, 0x90,
};

kal_uint16 addr_data_pair_normal_video_macro[] = {
	0xfe, 0x00,
	0x3e, 0x90,
};

kal_uint16 addr_data_pair_hs_video_macro[] = {
	0xfe, 0x00,
	0x3e, 0x90,
};

kal_uint16 addr_data_pair_slim_video_macro[] = {
	0xfe, 0x00,
	0x3e, 0x90,
};
static void sensor_init(void)
{
	...
	table_write_cmos_sensor(addr_data_pair_init_macro,
		sizeof(addr_data_pair_init_macro) / sizeof(kal_uint16));
        	...
        	do {
            	*sensor_id = return_sensor_id();
            	if (*sensor_id == 0x02e0) {
                    ...
                    if (module_id == IMGSENSOR_GC02M1_OTP_MODULE_ID)
                    {
                        if(check_material_id() == 1)
                        {
                            *sensor_id = imgsensor_info.sensor_id;
                	        LOG_INF("gc02m1b_macro get_imgsensor_id success: 0x%x\n",*sensor_id);
                	        return ERROR_NONE;
                        }
                        *sensor_id = 0;
                        LOG_INF("gc02m1b_macro get_imgsensor_id fail: 0x%x\n",*sensor_id);
                    }
                    else
                    {
                        *sensor_id = 0;
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            ...
            if (sensor_id == 0x02e0) {
                sensor_id = imgsensor_info.sensor_id;
                LOG_INF("gc02m1b open success: 0x%x\n",sensor_id);
                break;
            }
            LOG_INF("Read sensor id fail, id: 0x%x\n",sensor_id);
            retry--;
...
UINT32 GC02M1B_MIPI_RAW_MACRO_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
    ...
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    GC02M1B_MIPI_RAW_MACRO_SensorInit    */
